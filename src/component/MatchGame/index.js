...
class MatchGame extends Component {
  ...
  checkImageMatch = id => {
    ...
    if (matchImage.id === id) {
      ...
      this.randomImagePick()
    } else {
      this.setState({isGameRunning: false, count: 0})
    }
  }
  ...
  renderGame = () => {
    ...
    return (
      <div className="game-container">
        <img className="match-img" src={matchImage.imageUrl} alt="match" />
  ...
  render() {
    const {score, count, isGameRunning} = this.state
    return (
